    ...
    <script>
        $(function(){
            ...
            $('.select').on('ifChecked', function () {
                var id = $(this).data('id');
                $('.query_' + id).iCheck('check');
            });

            $('.select').on('ifUnchecked', function () {
                var id = $(this).data('id');
                $('.query_' + id).iCheck('uncheck');
            });
        });
    </script>
@endsection
                    ...
                    <table class="table table-striped">
                        <tbody>
                            ...
                            @if(count($permission_list))
                                ...
                                <td>{{$item['name']}}</td>
                                <td>
                                    <input type="checkbox" class="minimal select {{ 'query_' . $item['pid'] }}" data-id="{{$item['id']}}" name="perm_ids[]" @if(in_array($item['id'], $role_perms_ids)) checked="checked" @endif value="{{$item['id']}}">
                                </td>
                            </tr>
                                @endforeach
                            @endif
