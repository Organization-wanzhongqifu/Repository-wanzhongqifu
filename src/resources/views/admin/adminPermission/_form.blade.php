@section('css')
    <link rel="stylesheet" href="{{ secure_asset('static/admin/plugins/bootstrap-iconpicker/icon-fonts/font-awesome-4.2.0/css/font-awesome.min.css') }}" />
...
<div class="form-group @if(session('errors.name')) has-error @endif">
    ...
    <div class="col-md-5">
        <input type="text" name="name" id="name" class="form-control" value="{{ $form_type == 'edit' && !old('name') ? $permission->name : old('name') }}" placeholder="请输入权限名称">
...
<div class="form-group @if(session('errors.rule')) has-error @endif">
    ...
    <div class="col-md-5">
        <input type="text" name="rule" id="rule" class="form-control" value="{{ $form_type == 'edit' && !old('rule') ? $permission->rule : old('rule') }}" placeholder="请输入权限规则">
