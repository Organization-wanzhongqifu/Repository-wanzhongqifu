    ...
    <script>
        ...
        $(function(){
            ...
            dataTable = $("#dataTable").DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '/adminApi/providers',
                    data: function (d) {
                        d.search = $('#search').val();
                        d.service = $('#choose_service').val();
                ...
                columns: [
                    ...
                    {data: 'service_province', name: 'service_province', orderable: false, render: function (data, type, row) {
                        var city = row['service_city'] === null ? '' : row['service_city'];
                        var province = row['service_province'] === null ? '' : row['service_province'];
                        var district = row['service_district'] === null ? '' : row['service_district'];
            ...
            $('body').on('click', '.delete', function () {
                ...
                layer.confirm('确定删除？', {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '/adminApi/providers/' + id + '/delete'
                    }).done(function (data) {
            ...
            $('#choose_service').on('change', function(e) {
                var id = $(this).val();
                if (parseInt(id) !== 0) {
                    dataTable.draw();
                    e.preventDefault();
            ...
            $('#excel').on('change', function (e) {
                ...
                $.ajax({
                    url: "{{ url('adminApi/providers/import') }}",
                    type: 'POST',
                    cache: false,
                    data: formData,
                    processData: false,
                ...
                layer.open({
                    ...
                    ,yes: function (index, layero) {
                        ...
                        if (arrive_time && leave_time) {
                            layer.msg('导出成功');
                            layer.close(index);
                            window.location.href = '{{ url('adminApi/providers/export') }}' + '?begin_time='+arrive_time+'&end_time='+leave_time;
                        } else {
    ...
    <div class="box">
        <div class="box-body table-responsive">
            <table id="dataTable" class="table table-bordered table-striped">
                ...
                <tr>
                    <th colspan="10">
                        <a href="{{ url('admin/providers/create') }}" class='btn btn-success btn-sm'>新增</a>
